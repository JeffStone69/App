#!/bin/bash
# XForge Trader v9.2 – Ultimate Full-Integration Installer + Push
set -e
cd "$(dirname "$0")"

echo "=== [1/9] Installing GitHub CLI (if needed) ==="
...
echo "=== [2/9] GitHub Browser Login (Google sign-in supported) ==="
gh auth login --web --git-protocol https

echo "=== [3/9] Creating clean professional structure ==="
mkdir -p data logs modules
rm -f SIM.py shipping.py xforge_historical_db.py launch.command 2>/dev/null || true

echo "=== [4/9] Writing requirements.txt & setup.py ==="
...
echo "=== [5/9] Writing COMPLETE modules/SIM.py with ALL functions integrated ==="
...
echo "=== [6/9] Creating launch.command, memory.txt, installer.sh ==="
...
All 9 tabs + RSI + Bollinger + Profit Score + full integrations included
EOF
...
git add .
git commit -m "XForge Trader v9.2 – Complete professional release (ALL functions integrated)" 2>/dev/null || true
git branch -M main
...
echo ""
echo "✅ DONE! Every single function is now integrated into your final GUI XForge."
