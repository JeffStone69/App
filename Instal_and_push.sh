#!/bin/bash
# install_and_push.sh – Full App Installer + Repair + GitHub Push
set -e
cd "$(dirname "$0")"

echo "=== [1/8] Full local repair & structure creation ==="
mkdir -p data logs modules

# Clean any old flat files
rm -f SIM.py shipping.py xforge_historical_db.py launch.command 2>/dev/null || true

echo "=== [2/8] Creating requirements.txt & setup.py ==="
...
setup(name="XForge-Trader", version="9.2.0",
      install_requires=["pandas","numpy","yfinance","gradio","matplotlib"],
      python_requires=">=3.9")
EOF

echo "=== [3/8] Writing hardened modules/SIM.py (RSI + Bollinger + Profitability) ==="
...
echo "=== [4/8] Creating launch.command ==="
...
echo "=== [5/8] Creating memory.txt (permanent anchor) ==="
cat > memory.txt << 'EOF'
XFORGE TRADER v9.2 – PERMANENT MEMORY ANCHOR
Repo: https://github.com/JeffStone69/App/
Date: May 2026
Status: Fully restored, clean professional structure, zero bloat
CLEAN FOLDER STRUCTURE: modules/, data/, logs/
KEY FEATURES: 9-tab Gradio dashboard with Live Tickers (RSI + Bollinger + Profit Score), all integrations
EOF

echo "=== [6/8] Creating standalone installer.sh ==="
...
echo "=== [7/8] Git operations – clean push to GitHub ==="
git init 2>/dev/null || true
git add .
git commit -m "XForge Trader v9.2 – Complete professional release (RSI, Bollinger, Profitability, 9 tabs, full installer)" 2>/dev/null || true
git branch -M main 2>/dev/null || true
git remote remove origin 2>/dev/null || true
git remote add origin https://github.com/JeffStone69/App.git
git push -u origin main --force

echo ""
echo "✅ SUCCESS! Repository is now fully up to date on GitHub."
echo "Your repo now contains the complete enhanced app."
echo ""
echo "Next steps for anyone:"
echo "  git clone https://github.com/JeffStone69/App.git"
echo "  cd App"
echo "  chmod +x installer.sh launch.command"
echo "  ./installer.sh"
echo "  ./launch.command"
